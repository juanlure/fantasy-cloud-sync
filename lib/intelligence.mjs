...
export function buildIntelligence({capturedAt,seasonStart,ownId,settings,rows,profiles,catalog,events}){
 ...
 for(const profile of profiles){
  ...
  for(const owned of profile.players){
   ...
   targets.push({playerId:owned.id,name:player.name,position:positionName[player.position]||'OTRA',club:catalog.teams?.[player.teamID]?.name||'',points:player.points??0,marketValue,priceChange:player.priceIncrement??0,clause,ownerId:profile.id,ownerName:seller.name,ownerBalance:seller.balance,onSale:!!market,askingPrice:market?.price??null,withinBidLimit:clause<=ownLimit.maxBid,cashShortfall:Math.max(0,clause-own.balance),pointsPerMillion:clause>0?Math.round((player.points??0)*10000000/clause)/10:0,clausePremiumPct:marketValue>0?Math.round((clause/marketValue-1)*100):null,lockedUntil:settings.clauseActivationDelay&&owned.owner.date?owned.owner.date+settings.clauseActivationDelay*86400:null});
  }
 }
 targets.sort((a,b)=>b.pointsPerMillion-a.pointsPerMillion||b.points-a.points);
 const deals=[];
