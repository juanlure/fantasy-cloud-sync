...
export async function fetchSnapshot(token, fetcher=fetch){
 ...
 try{
  const slug=typeof league.competition==='string'?league.competition:league.competition?.slug,score=league.scoreID;
  ...
  const missing=[...new Set(profiles.flatMap(p=>p.players.map(item=>item.id)).filter(id=>!catalog.players[id]))];
  for(const id of missing){
   const detail=await api('players/'+slug+'/'+id+'?lang=es&score='+score);
   catalog.players[id]={...detail,teamID:detail.team?.id??null,points:null};
  }
 ...
 try{snapshot.intelligence=profiles&&catalog?buildIntelligence({...snapshot,settings:league.settings,standings:league.standings,profiles,catalog,events}):{available:false,reason:insightError};}
